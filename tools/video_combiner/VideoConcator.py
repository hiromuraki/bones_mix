...
def concat_videos(videos: list[tuple[Path, str]], output_video_file: Path | str):
    if not videos:
        print("警告：传入的视频列表为空。")
        return
...
for i in [1, 2, 3, 4]:
    base_dir = f"/home/user/bones/sample_output/sample_{i}"
    concat_videos(
        [
            (Path(f"{base_dir}/2d_kp_rtmpose_h36m.mp4"), "2D (RTMPose)"),
            (Path(f"{base_dir}/3d_kp_rtmpose_to_mhformer.mp4"), "3D (MHFormer)"),
            (Path(f"{base_dir}/3d_kp_rtmpose_to_motionagformer.mp4"), "3D (MotionAGFormer)"),
        ],
        output_video_file=Path(f"{base_dir}/combined.mp4")
    )
